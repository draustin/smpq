        ...
        try:
            while self.retire_task_num is None or (self.num_tasks_done<self.retire_task_num):
                ...
                try:
                    ...
                    logger.exception('Worker process %s caught an exception during execution and pickling of results of task task_id %s.',self.name,task_id)
...
class Manager:
    def __init__(self,num_workers,sleep=time.sleep,protocol=None,retire_task_num=None):
        """
        ...
        Args:
            ...
            retire_task_num (int): number of tasks after which workers 'retire'. Useful for dealing with slow memory
                leaks. It takes a few seconds to make a worker, so don't set this too low.
        """
    ...
    def add_task(self,id,task,warning_filters=[]):
        ...
        t2=time.time()
        logger.log(3,'Added task %s with dumped size %g MB. dumps took %g s, put took %g s. tasks.qsize()=%d.',id,
                   len(task_dumped)/1e6,t1-t0,t2-t1,self.tasks.qsize())

    def map(self,func,args,warning_filters=[]):
        """Apply function series of arguments and return the result.
        ...
        while not (args is None and next_result_index==num_tasks_added):
            ...
            while self.tasks.qsize()<self.num_workers and args is not None:
                try:
                    index,arg=next(args)
            ...
            while True:
                ...
                logger.log(2,'Appended result of %d to unyielded results, whose length is now %d.',index,len(unyielded_results))
            # Yield results in the correct order.
            while True:
                try:
                    results_i,value=next(
                        (i,value) for i,(result_index,value) in enumerate(unyielded_results) if result_index==next_result_index)
                except StopIteration:
                    break
            ...
            if now-last_log_time>log_wait_time:
                ...
                logger.log(2,'Waiting, self.tasks.qsize()=%d.',self.tasks.qsize())
    ...
    def block_until_tasks_done(self):
        logger.log(2,'self.tasks.qsize()=%d. Calling self.tasks.join...',self.tasks.qsize())
        self.tasks.join()
    ...
    def get_result(self):
        ...
        if isinstance(value,Exception):
            ...
            raise value
        t2=time.time()
        logger.log(2,'Got result task_id %s, size %g MB. get_nowait took %g s, loads took %g s.',task_id,len(value_dumped)/1e6,
                   t1-t0,t2-t1)
